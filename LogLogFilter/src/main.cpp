...
void item_frequency()
{
	...
	auto cu = new CUSketch<2 * 1024 * 1024, 3>();
	...
	uint32_t key;
	int frequency;
	double tot_are_cu = 0.0;
	double tot_are_lf = 0.0;
	int itemnum = 0;
	while (fscanf(fg, "%u\t%d\n", &key, &frequency) == 2) {
		int fre_cu = cu->query(key);
		double fre_lf = lf_cu->query(key);
		tot_are_cu += abs(fre_cu - frequency) / (double)frequency;
	...
	cout << "CU AARE: " << tot_are_cu / itemnum << endl;
